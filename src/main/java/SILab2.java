    ...
    public List<Integer> function(List<Time> timesList) { //1
        ...
        for (int i = 0; i < timesList.size(); i++) { //3
            ...
            if (hr < 0 || hr > 24){ //7
                ...
                if (min < 0 || min > 59) //14
                    throw new RuntimeException("The minutes are not valid!"); //15
                else { //16
                    if (sec >= 0 && sec <= 59) //17
                        result.add(hr * 3600 + min * 60 + sec); //18
                    else //19
                        throw new RuntimeException("The seconds are not valid"); //20
                } //21
            }  //22
            else if (hr == 24 && min == 0 && sec == 0) { //23
                    result.add(hr * 3600 + min * 60 + sec); //24
            } 
            else { //25
